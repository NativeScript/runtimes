...
#define CHECK_NEW_FROM_UTF8_LEN(env, result, str, len)                         \
  do {                                                                         \
    ...
    auto str_maybe = v8::String::NewFromUtf8((env)->isolate, (str),            \
...
#define CHECK_NEW_STRING_ARGS(env, str, length, result)               \
  do {                                                                \
    ...
    RETURN_STATUS_IF_FALSE(                                           \
        (env), ((length) == NAPI_AUTO_LENGTH) || (length) <= INT_MAX, \
        napi_invalid_arg);                                            \
  } while (0)
...
napi_status NewString(napi_env env, const CCharType* str, size_t length,
                      napi_value* result, StringMaker string_maker) {
...
napi_status NewExternalString(napi_env env, CharType* str, size_t length,
                              napi_finalize finalize_callback,
                              void* finalize_hint, napi_value* result,
                              bool* copied, CreateAPI create_api,
  ...
  if (status == napi_ok) {
    ...
    if (finalize_callback) {
      env->CallFinalizer(finalize_callback, static_cast<CharType*>(str),
                         finalize_hint);
    }
  }
#else
...
class TrackedStringResource : public Finalizer, RefTracker {
 public:
  TrackedStringResource(napi_env env, napi_finalize finalize_callback,
...
class ExternalOneByteStringResource
    : public v8::String::ExternalOneByteStringResource,
      TrackedStringResource {
 public:
  ExternalOneByteStringResource(napi_env env, char* string, const size_t length,
                                napi_finalize finalize_callback,
                                void* finalize_hint)
...
class ExternalStringResource : public v8::String::ExternalStringResource,
                               TrackedStringResource {
 public:
  ExternalStringResource(napi_env env, char16_t* string, const size_t length,
                         napi_finalize finalize_callback, void* finalize_hint)
      : TrackedStringResource(env, finalize_callback, string, finalize_hint),
...
inline napi_status ConcludeDeferred(napi_env env, napi_deferred deferred,
                                    napi_value result, bool is_resolved) {
  NAPI_PREAMBLE(env);
  CHECK_ARG(env, result);
...
inline napi_status Unwrap(napi_env env, napi_value js_object, void** result,
                          UnwrapAction action) {
  NAPI_PREAMBLE(env);
  ...
  if (obj->IsProxy()) {
    v8::Local<v8::Proxy> proxy = obj.As<v8::Proxy>();
    obj = proxy->GetTarget().As<v8::Object>();
    // printf("proxy works\n");
  }
    ...
    if (isInternalField) {
      ...
      auto pkey = v8::Private::ForApi(
          env->isolate, v8::String::NewFromUtf8(env->isolate, "napi_private")
                            .ToLocalChecked());
      CHECK(obj->DeletePrivate(context, pkey).FromJust());
    }
...
class CallbackBundle {
 public:
  ...
  static inline v8::Local<v8::Value> New(napi_env env, napi_callback cb,
                                         void* data) {
...
class FunctionCallbackWrapper : public CallbackWrapperBase {
 public:
  ...
  static inline napi_status NewFunction(napi_env env, napi_callback cb,
                                        void* cb_data,
  ...
  static inline napi_status NewTemplate(
      napi_env env, napi_callback cb, void* cb_data,
...
inline napi_status Wrap(napi_env env, napi_value js_object, void* native_object,
  ...
  if (result != nullptr) {
    ...
    reference =
        v8impl::Reference::New(env, obj, 0, v8impl::Ownership::kUserland,
                               finalize_cb, native_object, finalize_hint);
    *result = reinterpret_cast<napi_ref>(reference);
  } else {
    // Create a self-deleting reference.
    reference = v8impl::Reference::New(
        env, obj, 0, v8impl::Ownership::kRuntime, finalize_cb, native_object,
        finalize_cb == nullptr ? nullptr : finalize_hint);
  }

  // REMOVEME: log the object we're wrapping
  // v8::String::Utf8Value utf8(env->isolate, obj);
...
TrackedFinalizer* TrackedFinalizer::New(napi_env env,
                                        ...
                                        void* finalize_data,
                                        void* finalize_hint) {
  return new TrackedFinalizer(env, finalize_callback, finalize_data,
                              finalize_hint);
}
...
void TrackedFinalizer::Finalize() { FinalizeCore(/*deleteMe:*/ true); }
...
RefBase::RefBase(napi_env env, uint32_t initial_refcount, Ownership ownership,
...
RefBase* RefBase::New(napi_env env, uint32_t initial_refcount,
                      Ownership ownership, napi_finalize finalize_callback,
                      void* finalize_data, void* finalize_hint) {
  return new RefBase(env, initial_refcount, ownership, finalize_callback,
                     finalize_data, finalize_hint);
}

void* RefBase::Data() { return finalize_data_; }

uint32_t RefBase::Ref() { return ++refcount_; }
...
uint32_t RefBase::RefCount() { return refcount_; }
...
Reference* Reference::New(napi_env env, v8::Local<v8::Value> value,
                          uint32_t initial_refcount, Ownership ownership,
                          napi_finalize finalize_callback, void* finalize_data,
                          void* finalize_hint) {
  return new Reference(env, value, initial_refcount, ownership,
...
napi_status NAPI_CDECL napi_create_function(napi_env env, const char* utf8name,
  ...
  STATUS_CALL(v8impl::FunctionCallbackWrapper::NewFunction(env, cb,
                                                           callback_data, &fn));
...
napi_status NAPI_CDECL napi_define_class(
  ...
  *result = v8impl::JsValueFromV8LocalValue(
  ...
  if (static_property_count > 0) {
    ...
    STATUS_CALL(napi_define_properties(env, *result, static_descriptors.size(),
...
napi_status NAPI_CDECL napi_get_property_names(napi_env env, napi_value object,
                                               napi_value* result) {
  // Implementation from:
  // https://github.com/nodejs/node/blob/57351b628cae6167f03c0417a5e2334da574a743/src/js_native_api_v8.cc
  NAPI_PREAMBLE(env);
  CHECK_ARG(env, result);
...
napi_status NAPI_CDECL napi_set_property(napi_env env, napi_value object,
  ...
  RETURN_STATUS_IF_FALSE_WITH_PREAMBLE(env, set_maybe.FromMaybe(false),
                                       napi_generic_failure);
  return GET_RETURN_STATUS(env);
}
...
napi_define_properties(napi_env env, napi_value object, size_t property_count,
  ...
  for (size_t i = 0; i < property_count; i++) {
    ...
    if (p->getter != nullptr || p->setter != nullptr) {
      ...
      RETURN_STATUS_IF_FALSE_WITH_PREAMBLE(env, define_maybe.FromMaybe(false),
                                           napi_invalid_arg);
    } else if (p->method != nullptr) {
...
napi_status NAPI_CDECL napi_object_freeze(napi_env env, napi_value object) {
  ...
  RETURN_STATUS_IF_FALSE_WITH_PREAMBLE(env, set_frozen.FromMaybe(false),
                                       napi_generic_failure);
...
napi_status NAPI_CDECL napi_object_seal(napi_env env, napi_value object) {
  ...
  RETURN_STATUS_IF_FALSE_WITH_PREAMBLE(env, set_sealed.FromMaybe(false),
                                       napi_generic_failure);
...
napi_status NAPI_CDECL node_api_create_external_string_utf16(
    napi_env env, char16_t* str, size_t length, napi_finalize finalize_callback,
    void* finalize_hint, napi_value* result, bool* copied) {
  return v8impl::NewExternalString(
      env, str, length, finalize_callback, finalize_hint, result, copied,
...
napi_status NAPI_CDECL node_api_symbol_for(napi_env env,
  ...
  STATUS_CALL(napi_create_string_utf8(env, utf8description, length,
...
napi_status NAPI_CDECL napi_wrap(napi_env env, napi_value js_object,
  ...
  return v8impl::Wrap(env, js_object, native_object, finalize_cb, finalize_hint,
                      result);
}
...
napi_status NAPI_CDECL napi_create_external(napi_env env, void* data,
  ...
  if (finalize_cb) {
    ...
    // callback.
    v8impl::Reference::New(env, external_value, 0, v8impl::Ownership::kRuntime,
...
// added preprocessor for NAPI_VERSION check. napi_type_tag only defined for
// NAPI_VERSION >= 8
#if NAPI_VERSION >= 8
...
napi_status NAPI_CDECL napi_type_tag_object(napi_env env, napi_value object,
  ...
  RETURN_STATUS_IF_FALSE_WITH_PREAMBLE(env, !maybe_has.FromJust(),
                                       napi_invalid_arg);
  ...
  RETURN_STATUS_IF_FALSE_WITH_PREAMBLE(env, maybe_set.FromJust(),
                                       napi_generic_failure);
...
napi_status NAPI_CDECL napi_check_object_type_tag(napi_env env,
  ...
  *result = false;
  if (val->IsBigInt()) {
    ...
    int size = 2;
    napi_type_tag tag;
    val.As<v8::BigInt>()->ToWordsArray(&sign, &size,
...
napi_status NAPI_CDECL napi_create_external_arraybuffer(
  ...
  // The sandbox requires every ArrayBuffer backing store to live inside the
  // sandbox's own address space, so a caller-owned pointer cannot back one.
  // Node errors out here; copy instead, and keep the finalizer contract by
  // attaching a weak finalizer to the JS ArrayBuffer.
  //
  // The prebuilt slices are currently built with the sandbox off (it comes with
  // pointer compression, which is disabled for exactly this reason), so this
  // arm is not the one in use -- but the copy is the only correct behaviour if
  // that ever changes.
...
#else
  // Genuinely external: V8 points at the caller's memory and calls Finalize
  // when the backing store dies, which is the documented Node-API contract.
  struct FinalizeData {
    napi_env env;
    napi_finalize finalize_cb;
    void* finalize_hint;
...
napi_status napi_run_script_source(napi_env env, napi_value script,
  ...
  source_url_comment << std::endl
                     << "//# sourceURL=" << source_url << std::endl;
  const auto source_url_comment_str = source_url_comment.str();

  const auto v8_script_string =
      v8::Local<v8::String>::Cast(v8impl::V8LocalValueFromJsValue(script));
  const auto source_with_comment = v8::String::Concat(
      env->isolate, v8_script_string,
      OneByteString(env->isolate, source_url_comment_str.c_str(),
...
napi_status NAPI_CDECL napi_run_script_as_module(napi_env env,
  ...
  v8impl::RegisterESModule(isolate, modulePath, module);
...
napi_status NAPI_CDECL napi_set_instance_data(napi_env env, void* data,
                                              napi_finalize finalize_cb,
                                              void* finalize_hint) {
  CHECK_ENV(env);
  ...
  env->instance_data = v8impl::RefBase::New(env, 0, v8impl::Ownership::kRuntime,
                                            finalize_cb, data, finalize_hint);
...
napi_status NAPI_CDECL napi_detach_arraybuffer(napi_env env,
  ...
  v8::Local<v8::Value> value = v8impl::V8LocalValueFromJsValue(arraybuffer);
  RETURN_STATUS_IF_FALSE(env, value->IsArrayBuffer(),
                         napi_arraybuffer_expected);

  v8::Local<v8::ArrayBuffer> it = value.As<v8::ArrayBuffer>();
  RETURN_STATUS_IF_FALSE(env, it->IsDetachable(),
...
// object itself and the `data` pointer. There is no JS target and no prototype
// fiddling at this layer (callers do that); this keeps the proxy fully native.
class NapiHostObject {
 public:
  // Internal field 0 holds the wrapper; field 1 holds a process-unique sentinel
  // so a host object can be recognized by a fast internal-field read.
  enum InternalFields { kSlot = 0, kTypeSlot = 1, kInternalFieldCount = 2 };
  static void* TypeId() {
    static char id = 0;
    return &id;
  }

  static NapiHostObject* New(napi_env env, napi_finalize finalize, void* data,
                             const napi_host_object_methods* methods) {
    return new NapiHostObject(env, finalize, data, methods);
  }
  ...
  // Runs (via a Reference finalizer) after the host object is collected:
  // invokes the user finalizer (if any) and frees the wrapper.
  static void Finalize(napi_env env, void* data, void* /*hint*/) {
    auto* self = static_cast<NapiHostObject*>(data);
    if (self->finalize_ != nullptr) {
      self->finalize_(env, self->data_, nullptr);
    }
    delete self;
  }

  // ---------------- named interceptors ----------------
  static HOST_INTERCEPTED Getter(
      v8::Local<v8::Name> property,
      const v8::PropertyCallbackInfo<v8::Value>& info) {
    v8::Local<v8::Object> holder = info.Holder();
    auto* w = From(holder);
    napi_value host = JsValueFromV8LocalValue(holder);
    napi_value prop = JsValueFromV8LocalValue(property);
    napi_value result = nullptr;
    w->env_->CallIntoModule([&](napi_env env) {
      result = w->methods_.get(env, host, prop, w->data_);
    });
    if (result != nullptr) {
      info.GetReturnValue().Set(V8LocalValueFromJsValue(result));
    }
    HOST_HANDLED;
  }

  static HOST_INTERCEPTED Setter(v8::Local<v8::Name> property,
                                 v8::Local<v8::Value> value,
                                 HOST_SETTER_INFO info) {
    v8::Local<v8::Object> holder = info.Holder();
    auto* w = From(holder);
    napi_value host = JsValueFromV8LocalValue(holder);
    napi_value prop = JsValueFromV8LocalValue(property);
    napi_value val = JsValueFromV8LocalValue(value);
    w->env_->CallIntoModule(
        [&](napi_env env) { w->methods_.set(env, host, prop, val, w->data_); });
    HOST_SETTER_HANDLED;
  }

  static HOST_INTERCEPTED Query(
      v8::Local<v8::Name> property,
      const v8::PropertyCallbackInfo<v8::Integer>& info) {
    v8::Local<v8::Object> holder = info.Holder();
    auto* w = From(holder);
    if (w->methods_.has == nullptr) HOST_FALLTHROUGH;
    napi_value host = JsValueFromV8LocalValue(holder);
    napi_value prop = JsValueFromV8LocalValue(property);
    bool present = false;
    w->env_->CallIntoModule([&](napi_env env) {
      present = w->methods_.has(env, host, prop, w->data_);
    });
    if (!present) HOST_FALLTHROUGH;
    info.GetReturnValue().Set(v8::Integer::New(info.GetIsolate(), v8::None));
    HOST_HANDLED;
  }

  static HOST_INTERCEPTED Deleter(
      v8::Local<v8::Name> property,
      const v8::PropertyCallbackInfo<v8::Boolean>& info) {
    v8::Local<v8::Object> holder = info.Holder();
    auto* w = From(holder);
    if (w->methods_.delete_property == nullptr) HOST_FALLTHROUGH;
    napi_value host = JsValueFromV8LocalValue(holder);
    napi_value prop = JsValueFromV8LocalValue(property);
    bool deleted = false;
    w->env_->CallIntoModule([&](napi_env env) {
      deleted = w->methods_.delete_property(env, host, prop, w->data_);
    });
  ...
  static void Enumerator(const v8::PropertyCallbackInfo<v8::Array>& info) {
    v8::Local<v8::Object> holder = info.Holder();
    auto* w = From(holder);
    if (w->methods_.own_keys == nullptr) return;
    napi_value host = JsValueFromV8LocalValue(holder);
    napi_value keys = nullptr;
    w->env_->CallIntoModule([&](napi_env env) {
      keys = w->methods_.own_keys(env, host, w->data_);
    });
    if (keys == nullptr) return;
    v8::Local<v8::Value> keys_value = V8LocalValueFromJsValue(keys);
    if (keys_value->IsArray()) {
      info.GetReturnValue().Set(keys_value.As<v8::Array>());
    }
  }

  // ---------------- indexed interceptors ----------------
  static HOST_INTERCEPTED IndexedGetter(
      uint32_t index, const v8::PropertyCallbackInfo<v8::Value>& info) {
    v8::Local<v8::Object> holder = info.Holder();
    auto* w = From(holder);
    napi_value host = JsValueFromV8LocalValue(holder);
    napi_value result = nullptr;
    if (w->methods_.indexed_get != nullptr) {
      w->env_->CallIntoModule([&](napi_env env) {
        result = w->methods_.indexed_get(env, host, index, w->data_);
      });
    } else {
      napi_value prop = JsValueFromV8LocalValue(
          v8::Integer::NewFromUnsigned(info.GetIsolate(), index));
      w->env_->CallIntoModule([&](napi_env env) {
        result = w->methods_.get(env, host, prop, w->data_);
      });
    }
  ...
  static HOST_INTERCEPTED IndexedSetter(uint32_t index,
                                        v8::Local<v8::Value> value,
                                        HOST_SETTER_INFO info) {
    v8::Local<v8::Object> holder = info.Holder();
    auto* w = From(holder);
    napi_value host = JsValueFromV8LocalValue(holder);
    napi_value val = JsValueFromV8LocalValue(value);
    if (w->methods_.indexed_set != nullptr) {
      w->env_->CallIntoModule([&](napi_env env) {
        w->methods_.indexed_set(env, host, index, val, w->data_);
      });
    } else {
      napi_value prop = JsValueFromV8LocalValue(
          v8::Integer::NewFromUnsigned(info.GetIsolate(), index));
      w->env_->CallIntoModule([&](napi_env env) {
        w->methods_.set(env, host, prop, val, w->data_);
      });
    }
    HOST_SETTER_HANDLED;
  }

  static HOST_INTERCEPTED IndexedQuery(
  ...
  static HOST_INTERCEPTED IndexedDeleter(
 ...
 private:
  NapiHostObject(napi_env env, napi_finalize finalize, void* data,
                 const napi_host_object_methods* methods)
      : env_(env), finalize_(finalize), data_(data), methods_(*methods) {}

  ~NapiHostObject() = default;

  napi_env env_;
  napi_finalize finalize_;
  void* data_;
  napi_host_object_methods methods_;
};

#undef HOST_INTERCEPTED
#undef HOST_HANDLED
#undef HOST_FALLTHROUGH
#undef HOST_SETTER_INFO
#undef HOST_SETTER_HANDLED

}  // namespace v8impl

napi_status NAPI_CDECL napi_create_host_object(
    napi_env env, napi_finalize finalize, void* data,
    const napi_host_object_methods* methods, napi_value* result) {
  ...
  if (env->host_object_template.IsEmpty()) {
    ...
    tpl->SetInternalFieldCount(v8impl::NapiHostObject::kInternalFieldCount);

    v8::NamedPropertyHandlerConfiguration named(
        v8impl::NapiHostObject::Getter, v8impl::NapiHostObject::Setter,
        v8impl::NapiHostObject::Query, v8impl::NapiHostObject::Deleter,
        v8impl::NapiHostObject::Enumerator, v8::Local<v8::Value>(),
        v8::PropertyHandlerFlags::kNonMasking);
    tpl->SetHandler(named);

    v8::IndexedPropertyHandlerConfiguration indexed(
        v8impl::NapiHostObject::IndexedGetter,
        v8impl::NapiHostObject::IndexedSetter,
        v8impl::NapiHostObject::IndexedQuery,
        v8impl::NapiHostObject::IndexedDeleter, nullptr, nullptr, nullptr);
    tpl->SetHandler(indexed);

    env->host_object_template.Reset(isolate, tpl);
  }

  v8::Local<v8::ObjectTemplate> tpl = env->host_object_template.Get(isolate);
  v8::Local<v8::Object> hostObject = tpl->NewInstance(context).ToLocalChecked();

  auto* self = v8impl::NapiHostObject::New(env, finalize, data, methods);
  v8impl::NapiHostObject::Wrap(hostObject, self);

  *result = v8impl::JsValueFromV8LocalValue(hostObject);

  // Free the wrapper (and run the user finalizer, if any) once the host
  // object is collected. Registered unconditionally so the wrapper never
  // leaks.
  v8impl::Reference::New(env, hostObject, 0, v8impl::Ownership::kRuntime,
                         v8impl::NapiHostObject::Finalize, self, nullptr);

  return napi_clear_last_error(env);
}

napi_status NAPI_CDECL napi_get_host_object_data(napi_env env,
                                                 napi_value object,
                                                 void** data) {
  CHECK_ARG(env, object);
  CHECK_ARG(env, data);

  v8::Local<v8::Value> value = v8impl::V8LocalValueFromJsValue(object);
  *data = nullptr;
  if (value->IsObject()) {
    v8::Local<v8::Object> obj = value.As<v8::Object>();
    if (v8impl::NapiHostObject::IsHostObject(obj)) {
      *data = v8impl::NapiHostObject::From(obj)->Data();
    }
  }
  return napi_clear_last_error(env);
}

napi_status NAPI_CDECL napi_is_host_object(napi_env env, napi_value object,
                                           bool* result) {
  CHECK_ARG(env, object);
  CHECK_ARG(env, result);
  ...
  *result = value->IsObject() &&
            v8impl::NapiHostObject::IsHostObject(value.As<v8::Object>());
  return napi_clear_last_error(env);
}
