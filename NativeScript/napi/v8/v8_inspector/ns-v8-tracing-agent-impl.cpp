...
TracingAgentImpl::TracingAgentImpl() {
  tracing_controller_ = reinterpret_cast<TracingController*>(
      JSR::platform->GetTracingController());
}
...
void TracingAgentImpl::SendToDevtools(v8::Local<v8::Context> context,
                                      std::string jsonData) {
  std::string source = R"(
        (() => {
  ...
  bool success = v8::Script::Compile(context, tns::ToV8String(isolate, source))
                     .ToLocal(&script);
  ...
  success = processTraceData->Call(context, processTraceData, 1, args)
                .ToLocal(&result);
  assert(success);
}
