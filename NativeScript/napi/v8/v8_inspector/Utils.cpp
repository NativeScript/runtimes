...
Local<v8::Function> tns::inspector::GetDebuggerFunction(
    Local<Context> context, std::string domain, std::string functionName,
...
Local<v8::Function> tns::inspector::GetDebuggerFunctionFromObject(
    Local<Context> context, const Local<Object>& object,
    Local<Object>& domainDebugger) {
  Isolate* isolate = Isolate::GetCurrent();
  auto methodKey =
      v8::String::NewFromUtf8(isolate, "method", v8::NewStringType::kNormal)
          .ToLocalChecked();
  ...
  auto domainMethod =
      methodString.substr(domainSeparatorIndex + 1, methodString.size());
