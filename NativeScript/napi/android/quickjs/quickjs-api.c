...
static void *js_def_malloc(JSMallocState *s, size_t size)
{
    ...
    ptr = mi_heap_malloc((mi_heap_t *) s->opaque, size);
    if (!ptr)
        return NULL;
...
static void *js_def_realloc(JSMallocState *s, void *ptr, size_t size)
{
    ...
    ptr = mi_heap_realloc((mi_heap_t *) s->opaque, ptr, size);
    if (!ptr)
        return NULL;
...
napi_status napi_type_tag_object(napi_env env, napi_value object, const napi_type_tag *tag) {
    ...
    int hasTag = JS_GetOwnProperty(env->context, NULL, jsValue,
    ...
    int status = JS_SetProperty(env->context, jsValue, env->atoms.napi_typetag, value);
    if (status < 0) {
        return napi_set_last_error(env, napi_pending_exception, NULL, 0, NULL);
    }
...
napi_check_object_type_tag(napi_env env, napi_value object, const napi_type_tag *tag,
    ...
    size_t wordCount = 2;
    uint64_t words[2] = {0};
    bool converted = JS_GetBigIntWords(env->context, value, &sign, &wordCount, words);
    JS_FreeValue(env->context, value);
