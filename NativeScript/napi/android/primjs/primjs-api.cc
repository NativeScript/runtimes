...
static inline void js_exit(napi_env env) {
    if (--env->js_enter_state <= 0) {
        primjs_execute_pending_jobs(env);
    }
}
        ...
        if (code || code_cstring) {
            ...
            if (code == nullptr) {
                ...
                RETURN_STATUS_IF_FALSE(env,
                                       LEPUS_IsString(code_value) || LEPUS_IsUndefined(code_value),
                                       napi_string_expected);

                code_value = JS_DupValue_Comp(env->ctx->ctx, code_value);
            }

            if (!LEPUS_IsUndefined(code_value)) {
                CHECK_QJS(env, LEPUS_SetProperty(env->ctx->ctx, ToJSValue(error),
...
namespace qjsimpl {
    typedef struct FunctionDataWrapper {
        void *data;
    } FunctionDataWrapper;
}
...
napi_status napi_create_function(napi_env env, const char *utf8name,
    ...
    auto dataWrapper = new qjsimpl::FunctionDataWrapper();
    dataWrapper->data = callback_data;

    LEPUSValue data[] = {
            LEPUS_MKPTR(LEPUS_TAG_LEPUS_CPOINTER, env),
            LEPUS_MKPTR(LEPUS_TAG_LEPUS_CPOINTER, reinterpret_cast<void *>(cb)),
            LEPUS_MKPTR(LEPUS_TAG_LEPUS_CPOINTER, dataWrapper)};
    LEPUSValue fun = LEPUS_NewCFunctionData(
            ...
            [](LEPUSContext *ctx, LEPUSValueConst this_val, int argc,
               ...
               LEPUSValue *func_data) -> LEPUSValue {
                ...
                auto dataWrapper = reinterpret_cast<qjsimpl::FunctionDataWrapper *> LEPUS_VALUE_GET_CPOINTER(
                        func_data[2]);

                napi_clear_last_error(env);

                napi_handle_scope__ scope(env, env->ctx->ctx, reset_napi_env);
    ...
    napi_value external;
    napi_create_external(env, dataWrapper, [](napi_env, void *data, void *hint) {
        if (data) {
            auto dataWrapper = reinterpret_cast<qjsimpl::FunctionDataWrapper *>(data);
            delete dataWrapper;
        }
    }, dataWrapper, &external);
    napi_set_named_property(env, ToNapi(&fun), "__qjs::finalizer__", external);
...
napi_status napi_define_class(napi_env env, const char *utf8name, size_t length,
    ...
    qjsimpl::Value proto(ctx, LEPUS_NewObject(ctx));
    ...
    auto ctor_handle = env->ctx->CreateHandle(cfunction, true);
    ...
    *result = scope.Escape(ctor_handle);
...
napi_status napi_call_function(napi_env env, napi_value recv, napi_value func,
    ...
    js_exit(env);
    auto rt = LEPUS_GetRuntime(ctx);
    if (rt && !rt->current_stack_frame) {
        LEPUSContext *pctx = NULL;
        int result = 0;
        while ((result = LEPUS_ExecutePendingJob(rt, &pctx))) {
            if (result < 0) {
...
napi_status napi_throw_error(napi_env env, const char *code, const char *msg) {
    ...
    LEPUSValue code_val = LEPUS_UNDEFINED;
    if (code != NULL) {
        LEPUS_NewString(env->ctx->ctx, code);
...
napi_status napi_open_handle_scope(napi_env env, napi_handle_scope *result) {
    *result = reinterpret_cast<napi_handle_scope>(
            new napi_handle_scope__(env, env->ctx->ctx, reset_napi_env));
...
napi_status napi_close_handle_scope(napi_env env, napi_handle_scope scope) {
    ...
    delete reinterpret_cast<napi_handle_scope__ *>(scope);
    return napi_clear_last_error(env);
}
...
napi_status napi_escape_handle(napi_env env, napi_escapable_handle_scope scope,
    ...
    *result = reinterpret_cast<napi_handle_scope__ *>(scope)->Escape(escapee);
    return napi_clear_last_error(env);
}
...
struct napi_deferred__ {
    ...
    static napi_value Callback(napi_env env, napi_callback_info cbinfo) {
        napi_deferred__ *deferred =
                static_cast<napi_deferred__ *>(cbinfo->data);
...
napi_status napi_create_promise(napi_env env, napi_deferred *deferred,
    ...
    std::unique_ptr<napi_deferred__> deferred_val =
            std::make_unique<napi_deferred__>();
    napi_value executor{}, promise_val{};
    CHECK_NAPI(napi_create_function(env, "executor", NAPI_AUTO_LENGTH,
                                    napi_deferred__::Callback,
...
napi_status napi_run_script(napi_env env, napi_value script,
                            napi_value *result) {
    return napi_run_script_source(env, script, "<anonymous", result);
}

napi_status NAPI_CDECL napi_run_script_source(napi_env env,
                                              napi_value script,
    ...
    size_t size = 0;
    const char *src = LEPUS_ToCStringLen(env->ctx->ctx, &size, ToJSValue(script));
    js_enter(env);
...
inline napi_handle_scope__::napi_handle_scope__(napi_env env, LEPUSContext *ctx,
                                                napi_func *func)
...
inline napi_value napi_handle_scope__::CreateHandle(LEPUSValue v) {
    Handle *h = new Handle{.value = v, .prev = handle_tail_};
    handle_tail_ = h;
    return ToNapi(&(h->value));
}

inline napi_value napi_handle_scope__::Escape(napi_value v) {
...
napi_status primjs_execute_pending_jobs(napi_env env) {
    ...
    do {
        ...
        error = LEPUS_ExecutePendingJob(LEPUS_GetRuntime(env->ctx->ctx), &context);
        if (error == -1) {
            return napi_set_last_error(env, napi_pending_exception);
        }
    } while (error != 0);
