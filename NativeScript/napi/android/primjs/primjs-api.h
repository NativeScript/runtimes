...
struct napi_context__ {
    ...
    napi_context__(napi_env env, LEPUSContext *ctx)
            : env(env),
        ...
        LEPUSValue gc = LEPUS_NewCFunction(ctx, [](LEPUSContext *ctx, LEPUSValueConst this_val,
                                                   int argc, LEPUSValueConst *argv) -> LEPUSValue {
    ...
    napi_value CreateHandle(LEPUSValue v, bool only_gc = false) {
        if (LEPUS_IsGCMode(ctx)) {
            return static_cast<napi_handle_scope__ *>(GetNapiScope(ctx))->CreateHandle(v);
        } else if (!only_gc) {
    ...
    void set_handle_scope(napi_handle_scope__ *scope) { handle_scope = scope; }

private:
    int refs = 1;
    napi_handle_scope__ *handle_scope{};
    bool gc_enable;

    friend class napi_handle_scope__;
};
...
// Adapter for napi_finalize callbacks.
    ...
    public:
        ...
        enum EnvReferenceMode {
            kNoEnvReference, kKeepEnvReference
        };
