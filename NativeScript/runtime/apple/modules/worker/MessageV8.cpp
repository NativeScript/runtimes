...
void Assert(bool condition, Isolate* isolate = nullptr,
            std::string const& reason = "Assert failed") {
  assert(condition);
}
...
void ThrowDataCloneException(napi_env env, Local<Context> context,
  ...
  std::string messageStr = ToString(isolate, message);
  NativeScriptException except(env, messageStr, "DataCloneError");
  except.ReThrowToJS(env);
}
class SerializerDelegate : public v8::ValueSerializer::Delegate {
 public:
  ...
  void ThrowDataCloneError(Local<v8::String> message) override {
    ThrowDataCloneException(env_, context_, message);
  }
...
bool Message::Serialize(napi_env env, napi_value inputNapi) {
  v8::Isolate* isolate = env->isolate;
  v8::Local<v8::Context> context = env->context();
  v8::Local<v8::Value> input = v8impl::V8LocalValueFromJsValue(inputNapi);

  HandleScope handle_scope(isolate);
  v8::Context::Scope context_scope(context);

  // Verify that we're not silently overwriting an existing message.
  nativescript::Assert(main_message_buf_.is_empty());
  ...
  nativescript::Assert(data.first != NULL, isolate);
  main_message_buf_ =
...
napi_value Message::Deserialize(napi_env env) {
  ...
  if (deserializer.ReadHeader(context).IsNothing()) return nullptr;
  Local<Value> return_value;
  if (!deserializer.ReadValue(context).ToLocal(&return_value)) return nullptr;
  ...
  return v8impl::JsValueFromV8LocalValue(return_value);
}
