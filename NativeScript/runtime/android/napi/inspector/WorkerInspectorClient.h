 ...
 * Chrome's semantics).
 *
 * This is the napi (multi-engine hybrid) port: it takes the worker's napi_env
 * and reaches the underlying v8::Isolate through env->isolate / env->context(),
 * matching the fork's JsV8InspectorClient. The inspector itself is V8-only.
 */
class WorkerInspectorClient final : public v8_inspector::V8InspectorClient,
                                    public v8_inspector::V8Inspector::Channel {
    public:
        // Worker thread, with the worker env's isolate locked and its context
        // created.
        WorkerInspectorClient(int workerId, napi_env env, ALooper* workerLooper,
        ...
        // work; used by WorkerWrapper::Terminate together with the cooperative
        // looper quit.
...
#endif  // __V8__ && APPLICATION_IN_DEBUG
