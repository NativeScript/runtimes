...
JsV8InspectorClient::JsV8InspectorClient(napi_env _env)
    : env(_env),
      inspector_(nullptr),
      session_(nullptr),
      connection_(nullptr),
...
void JsV8InspectorClient::dispatchMessage(const std::string& message) {
    ...
    */
    ...
    Local<Value> arg;
    success = v8::JSON::Parse(context, tns::ConvertToV8String(env->isolate, message)).ToLocal(&arg);
    ...
    if(!arg.IsEmpty() && arg->IsObject()) {
        ...
        if(success) {
            auto requestId = arg.As<Object>()->Get(context, tns::ConvertToV8String(env->isolate, "id")).ToLocalChecked();
...
void JsV8InspectorClient::runMessageLoopOnPause(int context_group_id) {
    ...
    while (!terminated_) {
        JniLocalRef msg(jEnv.CallStaticObjectMethod(inspectorClass_, getInspectorMessageMethod_, connection_));
        if (!msg.IsNull()) {
            auto inspectorMessage = tns::jstringToString(msg);
...
void JsV8InspectorClient::sendNotification(std::unique_ptr<StringBuffer> message) {
    this->SendToFrontend(stringViewToString(env->isolate, message->string()));
}
...
void JsV8InspectorClient::SendToFrontend(const std::string& message) {
    ...
    JniLocalRef connection;
    {
        ...
        connection = JniLocalRef(jenv.NewLocalRef(connection_));
    }

    const std::string msg = MaybeRewriteSourceMapURL(message);
    try {
        // TODO: Pete: Check if we can use a wide (utf 16) string here
        JniLocalRef str(jenv.NewStringUTF(msg.c_str()));
        jenv.CallStaticVoidMethod(inspectorClass_, sendMethod_, (jobject) connection, (jstring) str);
...
JsV8InspectorClient* JsV8InspectorClient::GetInstance() {
    ...
    auto* created = new JsV8InspectorClient(Runtime::GetRuntime(0)->GetNapiEnv());
...
void JsV8InspectorClient::inspectorSendEventCallback(const FunctionCallbackInfo<Value>& args) {
    ...
    std::string message = tns::ConvertToString(arg);
...
void JsV8InspectorClient::sendToFrontEndCallback(const v8::FunctionCallbackInfo<v8::Value>& args) {
    ...
    try {
        ...
        if ((args.Length() > 0) && args[0]->IsString()) {
            ...
            std::string message = tns::ConvertToString(args[0]->ToString(context).ToLocalChecked());

            std::string level = "log";
            if (args.Length() > 1  && args[1]->IsString()) {
                level = tns::ConvertToString(args[1]->ToString(context).ToLocalChecked());
            }
        ...
        napi_env env = Runtime::GetRuntime(0)->GetNapiEnv();
        e.ReThrowToNapi(env);
...
void JsV8InspectorClient::consoleLogCallback(napi_env env, ConsoleAPIType method, const std::vector<v8::Local<v8::Value>>& args) {
#ifdef APPLICATION_IN_DEBUG
    ...
    // the root client.
    WorkerWrapper* worker = WorkerWrapper::FromEnv(env);
    if (worker != nullptr) {
...
void JsV8InspectorClient::registerModules() {
    ...
    // __inspector
    success = global->Set(context, tns::ConvertToV8String(isolate, "__inspector"), inspectorObject).FromMaybe(false);
    assert(success);

    // __registerDomainDispatcher
    success = v8::Function::New(context, registerDomainDispatcherCallback).ToLocal(&func);
    assert(success);
    success = global->Set(context, tns::ConvertToV8String(isolate, "__registerDomainDispatcher"), func).FromMaybe(false);
    assert(success);
...
bool JsV8InspectorClient::CallDomainHandlerFunction(Local<Context> context, Local<Function> domainMethodFunc, const Local<Object>& arg, Local<Object>& domainDebugger, Local<Value>& result) {
    ...
    Local<Value> params;
    success = arg.As<Object>()->Get(context, tns::ConvertToV8String(isolate, "params")).ToLocal(&params);
    ...
    if (tc.HasCaught()) {
        std::string error = tns::ToString(env->isolate, tc.Message()->Get());
...
std::string JsV8InspectorClient::GetReturnMessageFromDomainHandlerResult(const Local<Value>& result, const Local<Value>& requestId) {
    ...
    if(!result->IsObject()) {
        ...
        if(result->IsNullOrUndefined() || result->BooleanValue(env->isolate)) {
            return "{ \"id\":" + tns::ToString(env->isolate, requestId) + ", \"result\": {} }";
        }
