...
ObjCProtocol* ObjCBridgeState::getProtocol(napi_env env, MDSectionOffset offset) {
...
napi_value ObjCProtocol::jsConstructor(napi_env env, napi_callback_info cbinfo) {
...
ObjCProtocol::ObjCProtocol(napi_env env, MDSectionOffset offset) {
  ...
  while (next) {
    ...
    auto proto =
        bridgeState->getProtocol(env, protocolImpl + bridgeState->metadata->protocolsOffset);
    protocols.emplace(proto);
    ObjCClassMember::defineMembers(env, proto->members, proto->membersOffset, constructor);
  }
